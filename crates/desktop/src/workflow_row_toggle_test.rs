...
use gpui::{point, px, size, AppContext};
...
fn workflow_row_toggle_expands_and_collapses_children(cx: &mut gpui::TestAppContext) {
    ...
    let (app, cx) = cx.add_window_view(|window, cx| {
        ...
        store.save_orchestrator(&OrchestratorConfig {
    ...
    let wf_id = cx.update(|_, cx| {
        let id = app.read(cx).workflows[0].id();
        id
    });
