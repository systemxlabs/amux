...
- **系统提示词（只传话、不决策）**：编排 agent 的系统提示词明确其职责——按用户指令把指定内容转发给对应子会话，把子会话结果原样转述回工作流会话；不自行拆解步骤、不自行选择机器与 agent、不自行分支 / 重试 / 汇总，也不输出建议或结论；未在用户指令中指定的事项交由用户决定（回到 idle 等待）。
...
编排 agent 的文本输出即对子会话结果的转述，作为工作流会话的对话内容；它不自行汇总或下结论。
...
- **压缩方式**：用编排 agent 自身模型把最旧的轮次压缩为一段摘要；此后上下文 = 系统提示词（含工作流模板，模板不进入会话历史）+ 摘要 + 未压缩的近期轮次
