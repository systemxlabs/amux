...
打开新建会话视图时，实时拉取机器、agents 和常用工作目录，不定时刷新。

当在工作目录输入框输入时，实时拉取全部目录项（不包括文件）进行前缀匹配
- 如果输入 `/home/tom/`，则应拉取 `/home/tom/` 目录下所有目录项进行展示
- 如果输入 `/home/tom`，则应拉取 `/home/` 目录下所有目录项，展示与 `tom` 前缀匹配的项
...
终端每次打开时，从 Server 获取终端列表，不在列表中的终端从应用中移除掉。
