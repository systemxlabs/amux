...
| **编排智能体** | GUI 应用内置的编排 agent（rig 实现），驱动工作流会话 |
| **工作流会话** | 由编排智能体驱动的工作流会话，状态持久化于所属 GUI 应用本地，不跨应用共享 |
| **子会话** | 工作流所驱动的普通会话（PRD 称「关联普通会话」），挂载在工作流会话下 |
...
- **多设备共存**：任意数量的 GUI 应用可同时连接同一 server、查看并操作同一会话，互不踢出；各 GUI 应用独立加载会话数据；会话列表变更（创建 / 删除 / 重命名）经轮询同步，无专用变更通知。工作流会话例外：仅存于所属 GUI 应用本地，不跨设备共享（其他应用不可见、不可操作）；其子会话作为普通会话，仍可被多设备查看与操作。
...
- **启动**：GUI 应用加载本地数据；按机器注册表逐个连接已注册的机器 server；连接失败或运行期断线则标记为离线并指数退避重连。
- **关闭**：只断开与各 server 的 WebSocket；server、ACP server 及进行中的会话均不受影响。重新打开后按注册表重新连接并恢复工作流会话（不主动驱动）。

### 4.3 本地数据

- server 本地数据存放于 `~/.amux/server`：配置、**会话注册表**（SQLite 数据库 `amux.db`，存会话元数据，支持结构化查询与事务更新）与**会话历史存储**（`history/`，每会话一个事件日志文件）。
- GUI 应用本地数据存放于 `~/.amux/gui`：机器注册表（含各机器连接 token）、常用工作目录（各机器最近使用的工作目录）、快捷指令、Skills 注册表、工作流模板、编排智能体 API 配置、工作流会话状态。

## 5. 数据模型

会话数据指一个会话的全部内容：对话内容（用户消息 + agent 输出）与会话活动。本节统一描述它们的交付、存储与获取。
...
- **默认模型**：GUI 设置页（机器管理）可为每个 agent 配置默认模型，server 侧持久化，创建 ACP 会话时生效。
...
- **安装 / 更新 / 卸载**：复用会话能力——新建会话（对应机器与 agent）后经 prompt 让 agent 自行下载、安装、更新或卸载（快捷指令式 prompt，无专用协议）
- **查看已安装列表**：GUI 经 server 查询某 agent 已安装的 skills（agent 不支持时返回空列表）

## 8. 工作流

工作流由 **GUI 应用内置编排智能体** 驱动（rig 实现），基于会话原语实现：

### 8.1 运行模型

- **工作流会话**：工作流创建一个工作流会话（GUI 应用内置编排智能体，状态存 GUI 应用本地），与普通会话一样支持 prompt / 状态（idle / busy）——busy = 编排智能体调度中或任一子会话工作中；**支持 steer**——busy 时用户输入注入当前 turn（编排智能体为 GUI 应用内置，steer 由 GUI 应用自身实现，不依赖 ACP 能力）。
- **取消与恢复**：用户主动取消工作流会话 = 取消编排智能体当前 turn 并取消其全部子会话；取消（或执行异常）后用户可再次输入，作为普通 prompt 进入新 turn，恢复或重启工作流。
- **rig 单 turn 模式**：每个 turn 调用一次 rig `Agent::prompt`（不用 `multi_turn` 长循环）——下达指令后 turn 结束、**不阻塞等待子会话**；下一 turn 由自动推进（§8.4）或用户输入触发。编排智能体的会话操作定义为 rig 工具（§8.2）。
- **API 配置**：编排智能体的 LLM 调用经自定义 API 端点配置——Base URL、API key、模型名与 **API format**：`chat_completions` / `responses` / `messages`（分别对应 OpenAI Chat Completions、OpenAI Responses、Anthropic Messages API）；未配置时不创建工作流会话，GUI 引导至设置页。
- **系统提示词（只传话、不自主决策）**：编排智能体的系统提示词明确其职责——按用户指令把指定内容转发给对应子会话，把子会话结果原样转述回工作流会话；不自行拆解步骤、不自行选择机器与 agent、不创造执行计划之外的步骤、不自主变更目标；可执行执行计划中明确写出的条件分支（如「审查不通过则打回修改」）；未在用户指令中指定的事项交由用户决定（回到 idle 等待；需要人类判断时输出结论并等待用户输入）。

### 8.2 编排工具

编排智能体的工具即会话原语，经 GUI 应用既有的 server 连接与协议方法执行：

| 工具 | 用途 |
|---|---|
| `list_agents` | 已注册机器及各机器的 agent 列表：机器在线状态、agent 可用性——定位用户指令指定的机器与 agent |
| `list_sessions` | 本工作流的子会话列表（标题、状态、最近活跃、机器在线与否）——复用已有子会话、查询步骤进展 |
| `create_session` | 在指定机器以指定 agent 与工作目录创建子会话，返回会话 ID |
| `prompt_session` | 向子会话下发指令（idle 启动工作、忙时 steer） |
| `cancel_session` | 取消子会话进行中的工作 |
| `read_session_history` | 按窗口 / 游标读取子会话对话内容——转述子会话结果 |

编排智能体的文本输出即对子会话结果的转述，作为工作流会话的对话内容；不做执行计划之外的汇总或结论（需要人类判断时输出结论并回到 idle 等待，见 §8.1）。

### 8.3 上下文管理（auto compaction）

工作流会话长期运行、轮次持续增长，发送给模型的上下文（编排智能体自身上下文，与 ACP agent 的 compaction 无关）需自动压缩：

- **历史与上下文分离**：展示用对话历史完整持久化（§4.3），压缩只影响发送给模型的上下文
- **触发**：每个 turn 开始前估算上下文 token 数，超过模型上下文窗口的阈值比例时先压缩再调用
- **压缩方式**：用编排智能体自身模型把最旧的轮次压缩为一段摘要；此后上下文 = 系统提示词（含工作流模板，模板不进入会话历史）+ 摘要 + 未压缩的近期轮次
...
- **注入内容**：子会话标识（ID / 标题）+ turn 结束原因（完成 / 取消 / 错误，来自 prompt result）；步骤细节由编排智能体经 `read_session_history` 按需自取，注入保持简短
- **busy 时注入**：注入即向工作流会话输入——idle 启动新 turn，busy 时作为 steer 注入当前 turn（§8.1）；多个子会话同时完成时合并为一次注入
- **恢复**：GUI 重开后仅恢复工作流会话展示（对话历史 / 活动历史），状态为空闲——即使仍有子会话在运行，停止的工作流会话也不跟踪其子会话状态：不补注入，也不响应运行中子会话的 busy→idle 边沿；重新启动需用户手动输入（工作流随应用关闭停止，重启由用户触发）
