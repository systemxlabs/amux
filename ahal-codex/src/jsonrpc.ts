/**
 * 最小 JSON-RPC 2.0 客户端（codex app-server stdio 传输，NDJSON 帧）。
 * 零依赖：node:child_process + node:readline。
 */
...
export class JsonRpcClient {
  ...
  private handleLine(line: string) {
    ...
    if ("method" in msg && "id" in msg && typeof msg.id === "number") {
      // request from server (e.g. approval) — 未实现服务端请求处理
      return;
    }
    if ("method" in msg) {
