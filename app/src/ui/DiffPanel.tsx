...
export interface DiffPanelProps {
  store: MachineStore;
  meta: SessionMeta;
  /** 当前展开的视图：diff（工作区 diff）或 detail（会话详情）；由悬浮按钮切换（PRD §3.1） */
  view: "diff" | "detail";
  /** 折叠右侧面板 */
  onClose: () => void;
  /** 删除会话（永久；调用方负责确认与清理选中状态） */
  onDelete: () => void;
}
...
interface StatusResult {
  branch: string;
  changes: Change[];
  /** cwd 不是 git 仓库（server 返回标记；GUI 不提供 diff 按钮） */
  notRepo?: boolean;
}
...
export function DiffPanel(props: DiffPanelProps) {
  ...
  const showDiff = props.view === "diff";
  ...
  return (
    <aside className="diff-panel">
      <div className="panel-head">
        <span className="panel-title">{showDiff ? "Diff" : "会话详情"}</span>
        <button className="btn subtle small" onClick={props.onClose} title="折叠右侧面板">
          ✕
        </button>
      </div>
      {showDiff ? (
        <div className="diff-view">
          ...
          {status?.notRepo ? (
            <div className="empty-hint">当前工作区不是 git 仓库</div>
          ) : (
            <>
